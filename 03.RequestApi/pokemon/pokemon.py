...
def numero_do_pokemon(nome):
    ...
    url = f'http://pokeapi.co/api/v2/pokemon/{lowerName}/'
    dataGetter = requests.get(url)
    if dataGetter.status_code == 200:
        dici = dataGetter.json()
        return dici['id']
...
def color_of_pokemon(nome):
    if not isinstance(nome, str):
        ...
        lowername = nome.lower()
        url = f'http://pokeapi.co/api/v2/pokemon-species/{lowername}/'
    dataGetter = requests.get(url)
    if dataGetter.status_code == 200:
        dici = dataGetter.json()
    else:
...
def cor_do_pokemon(nome):
    ...
    pokeColor = dici['color']['name']
    
    if pokeColor in dic_cores.keys():
        return dic_cores[pokeColor]
...
def tipos_do_pokemon(nome):
    ...
    if len(dici['types']) > 1:
        for x in range(0, len(dici['types'])):
            pokeTypes.append(dici['types'][x]['type']['name'])
    else:
        pokeTypes.append(dici['types'][0]['type']['name'])
    
    for type in pokeTypes:
        if type in dic_tipos.keys():
            typesInPortuguese.append(dic_tipos[type])
...
def evolucao_anterior(nome):
    ...
    if dici['evolves_from_species']['name'] == None:
        return 'None'
    else:
